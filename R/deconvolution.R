...
#' @param sigma The standard deviation of the peaks.
#'
...
xrf_add_deconvolution_gls <- function(.spectra, .energy_kev = .data$.spectra$energy_kev,
                                      ...
                                      peaks = xrf_energies(), sigma = 0.07, .env = parent.frame()) {
...
#' @export
xrf_deconvolute_gaussian_least_squares <- function(energy_kev, response, peaks = xrf_energies(), sigma = 0.07,
                                                   energy_min_kev = -Inf, energy_max_kev = Inf) {
  ...
  responses <- peaks %>%
    dplyr::select("element", "energy_kev", "relative_peak_intensity") %>%
    dplyr::mutate(
      response_element = purrr::pmap(
        list(.data$energy_kev, .data$relative_peak_intensity),
        function(mu, height) gaussian_fun(!!energy_kev, mu = mu, sigma = !!sigma, height = height)
      )
    ) %>%
  ...
  # least squares estimation of coefficients (peak height of tallest peak)
  ...
  responses$peak_area <- responses$height * sigma * sqrt(2 * pi)
  responses$peak_area_se <- responses$height_se * sigma * sqrt(2 * pi)
...
xrf_add_deconvolution_fun <- function(.spectra, .fun, ..., .env = parent.frame()) {
  dots <- quos(...)
  .spectra$.deconvolution <- purrr::map(purrr::transpose(.spectra), function(spectrum) {
