...
response <- deconv$deconvolution %>%
  gather(element, response, -starts_with(".")) %>%
  left_join(deconv$peaks, by = "element")

ggplot(spec_simple, aes(energy_kev, smooth - baseline)) +
  geom_line(aes(y = cps), alpha = 0.2) +
  geom_line() +
  geom_line(aes(x = .energy_kev, y = response * height, col = element), data = response) +
  geom_line(aes(x = .energy_kev, y = .response_fit), data = deconv$deconvolution, col = "blue") +
  geom_hline(yintercept = 0, alpha = 0.2) +
  stat_xrf_peaks(element_list = "major", nudge_y = 50) +
  scale_y_sqrt()
```
...
spec_simple2 <- specs %>%
  ...
  pull(.spectra) %>%
  first() %>%
  filter(energy_kev < 20)

deconv <- deconvolute_gaussian_least_squares(
  spec_simple2$energy_kev, 
  spec_simple2$smooth - spec_simple2$baseline,
  peaks = xrf_energies("lake_sediment")
)
...
ggplot(spec_simple2, aes(energy_kev, smooth - baseline)) +
  ...
  geom_line(aes(x = .energy_kev, y = .response_fit), data = deconv$deconvolution, col = "blue") +
  geom_hline(yintercept = 0, alpha = 0.2) +
  stat_xrf_peaks(element_list = "lake_sediment", nudge_y = 20) +
  scale_y_sqrt()
```
